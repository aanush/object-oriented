...
public final class SynchronizedFutureValueCache<K, V> implements FutureValueCache<K, V> {
    ...
    public SynchronizedFutureValueCache(int cacheSize, Function<K, V> cacheFunction) {
    ...
    private final KeyAndFutureValue<K, V> getKeyAndFutureValue(K key) {
        cacheLock.lock();
        try {
            ...
            if (keyAndFutureValue != null) {
                ...
                keyAndFutureValue = new KeyAndFutureValue<>(key, executorService.submit(() -> cacheFunction.apply(key)));
    ...
    private final KeyAndFutureValue<K, V> updateKeyAndFutureValue(K key) {
        cacheLock.lock();
        try {
            KeyAndFutureValue<K, V> keyAndFutureValue = map.get(key);
            deque.remove(keyAndFutureValue);
    ...
    private final class KeyAndFutureValue<K, V> {
        ...
        public Future<V> getFutureValue() {
            return futureValue;
        }
